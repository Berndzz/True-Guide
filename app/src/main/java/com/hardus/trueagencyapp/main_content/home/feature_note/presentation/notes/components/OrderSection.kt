...
fun OrderSection(
    ...
    Column(
        ...
        Row(
            ...
            DefaultRadioButton(
                text = "Prospect",
                selected = noteOrder is NoteOrder.CategoryProspect,
                onSelect = { onOrderChange(NoteOrder.CategoryProspect(noteOrder.orderType)) }
            )
            Spacer(modifier = Modifier.width(8.dp))
            DefaultRadioButton(
                text = "Recruit",
                selected = noteOrder is NoteOrder.CategoryRecruit,
                onSelect = { onOrderChange(NoteOrder.CategoryRecruit(noteOrder.orderType)) }
            )
        }
        Spacer(modifier = Modifier.height(16.dp))
        Row(
